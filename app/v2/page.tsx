...
type Leg = { from: string; to: string; fast: Trip & { tolls: { amount: number; period: string; segments: string[] } }; free: Trip };
...
function RouteCard({ title, accent, trip, rates, multiplier, toll, manualToll, onManualToll }: { title: string; accent: "blue" | "emerald"; trip: Trip; rates: typeof defaults; multiplier: number; toll?: Leg["fast"]["tolls"]; manualToll?: string; onManualToll?: (value: string) => void }) {
  ...
  return <article className={`rounded-2xl border bg-slate-900/80 p-4 shadow-xl ${accent === "blue" ? "border-blue-500/45" : "border-emerald-500/40"}`}>
    ...
    {toll && <div className="mt-3 rounded-xl border border-amber-500/25 bg-amber-500/10 p-3 text-sm"><div className="flex items-center justify-between gap-2"><span className="text-amber-100">Платная дорога</span><strong className="text-amber-300">+ {money(tollAmount)}</strong></div><p className="mt-1 text-xs text-amber-100/65">Ориентировочно, легковой автомобиль без транспондера. В стоимость поездки не включено.</p><label className="mt-2 flex items-center gap-2 text-xs text-slate-300">Уточнить вручную:<input type="number" min="0" value={manualToll} placeholder={String(toll.amount)} onChange={(e) => onManualToll?.(e.target.value)} className="h-8 w-24 rounded-lg border border-slate-600 bg-slate-950 px-2 text-white outline-none"/> ₽</label></div>}
  </article>;
}
...
export default function V2Page() {
  ...
  const dualTotals = useMemo(() => result?.legs.map((leg) => ({ fast: leg.fast.meters / 1000, free: leg.free.meters / 1000 })) ?? [], [result]);
